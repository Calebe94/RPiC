...
static void strobe (const struct lcdDataStruct *lcd){
  GPIOWrite (lcd->strbPin, 1) ; usleep (50) ;
  GPIOWrite (lcd->strbPin, 0) ; usleep (50) ;
}
...
static void sendDataCmd (const struct lcdDataStruct *lcd, unsigned char data){
  ...
  if (lcd->bits == 4){
    ...
    for (i = 0 ; i < 8 ; ++i){
      GPIOWrite (lcd->dataPins [i], (myData & 1)) ;
      myData >>= 1 ;
    }
  }
  strobe (lcd) ;
...
static void put4Command (const struct lcdDataStruct *lcd, unsigned char command){
  ...
  for (i = 0 ; i < 4 ; ++i){
    GPIOWrite (lcd->dataPins [i], (myCommand & 1)) ;
    myCommand >>= 1 ;
  }
  strobe (lcd) ;
}
...
// Simple sanity checks
  ...
  if ((rows < 0) || (rows > 20))    return -1 ;
  if ((cols < 0) || (cols > 20))    return -1 ;
...
// Create a new LCD:
  ...
  GPIOExport(lcd->rsPin);  GPIODirection (lcd->rsPin,   OUTPUT) ;  GPIOWrite (lcd->rsPin,   0) ; 
  GPIOExport(lcd->strbPin);  GPIODirection (lcd->strbPin, OUTPUT) ;  GPIOWrite (lcd->strbPin, 0) ; 

  for (i = 0 ; i < bits ; ++i){
	GPIOExport(lcd->dataPins [i]);
    GPIODirection      (lcd->dataPins [i], OUTPUT) ;
    GPIOWrite (lcd->dataPins [i], 0) ;
  }
  delay_ms (35) ; // mS
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  if (bits == 4){
    ...
    put4Command (lcd, func >> 4) ; delay_ms (35) ;
    put4Command (lcd, func >> 4) ; delay_ms (35) ;
    put4Command (lcd, func >> 4) ; delay_ms (35) ;
    ...
    putCommand  (lcd, func     ) ; delay_ms (35) ;
    putCommand  (lcd, func     ) ; delay_ms (35) ;
    putCommand  (lcd, func     ) ; delay_ms (35) ;
  }
